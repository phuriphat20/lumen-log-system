...
const MultiSelectInput = ({
    ...
    const toggleOption = (val) => {
        ...
        if (safeSelectedValues.includes(val)) {
            newValues = safeSelectedValues.filter(v => v !== val);
        } else {
            newValues = [...safeSelectedValues, val];
        }
        onChange(newValues);
    };
    ...
    return (
        <div className='flex flex-col gap-2 w-full relative' ref={containerRef}>
            ...
            >
                {Icon && (
                    <div className={`shrink-0 ${safeSelectedValues.length > 0 ? 'text-[#3B82F6]' : 'text-slate-400'}`}>
                        <Icon size={16} />
                    </div>
                )}

                <div className="flex flex-nowrap h-8 gap-1.5 flex-1 overflow-x-auto overflow-y-hidden custom-scrollbar py-1">
                    {/* ✨ ใช้ safeSelectedValues แทน selectedValues */}
                    {safeSelectedValues.length === 0 ? (
                        <span className="text-sm text-slate-400 self-center">{placeholder}</span>
                    ) : (
                        safeSelectedValues.map(val => {
                            // ใช้ safeOptions ในการหา label
                            const option = safeOptions.find(opt => (opt.value || opt) === val);
                            const displayLabel = option?.label || option || val; // Fallback เป็น val ถ้าหาไม่เจอ
                            return (
                                <span
                                    key={val}
            ...
            {isOpen && (
                <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-white border border-[#E2E8F0] rounded-2xl shadow-xl z-50 py-2 max-h-64  overflow-y-auto custom-scrollbar animate-in fade-in slide-in-from-top-2">
                    ...
                    >
                        <span className={`text-sm transition-colors ${safeSelectedValues.length === 0 ? 'text-[#3B82F6] font-bold' : 'text-slate-600 group-hover:text-slate-900'}`}>
                            {placeholder} {/* ใช้ placeholder แทนคำว่า "แสดงทั้งหมด" เพื่อความยืดหยุ่น */}
                        </span>
                        {safeSelectedValues.length === 0 && (
